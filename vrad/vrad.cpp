...
float g_CloudAngularExtent=0.0;
...
extern bool	bCustomAngles;
//extern Vector PreventTeal( Vector color );
extern float RGBintensity( Vector color );

extern int LightForKey( entity_t *ent, char *key, Vector& intensity );
Vector extsunlight, extambient;
...
Vector InvertHue( Vector color )
{
	 printf("refl %.3f %.3f %.3f inverted to ", color.x,color.y,color.z );

	float oldbrt = RGBintensity( color );
	color = Vector(1,1,1) - color;
	float newbrt = RGBintensity( color );
	float brtincrease = newbrt/oldbrt;
	color /= brtincrease;
...
void CollectLight( Vector& total, float num )
{
	...
	for( i = uiPatchCount - 1; i >= 0; i-- )
	{
		...
		else if( !g_bSkipChildren )
		{
			...
			//  patch->totallight was erased and rewritten


			// patch->emitlight = s1 * child1->emitlight + s2 * child2->emitlight
			VectorScale( emitlight[patch->child1], s1, emitlight[i] );
			VectorMA( emitlight[i], s2, emitlight[patch->child2], emitlight[i] );
			//  emitlight was erased and rewritten
		}
...
void BounceLight (void)
{
	...
	while ( bouncing )
	{
		...
		CollectLight( added, i+1 );


		if  (  added.Length()  )
		{
			qprintf ("\tLastBounce %.f, %.f, %.f energy %.f\n", LastBounce[0], LastBounce[1], LastBounce[2], RGBintensity(LastBounce) );
			printf ("\tBounce #%i added RGB(%.f, %.f, %.f) energy %.f\n", i+1, added[0], added[1], added[2], RGBintensity(added) );
		}
		energystats+=added;
...
//	g_Patches.EnsureCapacity( MAX_PATCHES );
	...
	if ( g_bDumpRtEnv )
		WriteRTEnv("trace.txt");// Source Engine 2009\ep2\trace.txt

	// Build acceleration structure
	printf ( "Setting up ray-trace acceleration structure... ");
	float start = Plat_FloatTime();
	g_RtEnv.SetupAccelerationStructure();
	float end = Plat_FloatTime();//BUGBUG: disable StaticPropPolys
...
int ParseCommandLine( int argc, char **argv, bool *onlydetail )
{
	*onlydetail = false;
	...
	for( i=1 ; i<argc ; i++ )
	{
		...
		{
			g_bSkipPointlights = true;
		}
		else if ( !Q_stricmp(argv[i], "-SkipSpotlights" ) )
		{
		...
		{
			bDoDebug = true;//old test thing, let's try it; where is dump file?
		}*/
		else if ( !Q_stricmp(argv[i], "-ForceTexlights" ) )
		{
		...
		else if (!Q_stricmp(argv[i],"-TransferCap"))
		{
			if ( ++i < argc )
			{
		...
		else if (!Q_stricmp(argv[i],"-gamma"))
		{
			if ( ++i < argc )
			{
				g_fGammaScale = atof( argv[i] );
			}
			else
			{
				Warning("Error: expected a value after '-gamma'\n" );
				return 1.0f;
			}
		}
		else if (!Q_stricmp(argv[i],"-lightscale"))
		{
			if ( ++i < argc )
			{
				lightscale = atof( argv[i] );
			}
			else
			{
