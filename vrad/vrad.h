...
struct LightingValue_t
{
	...
	{
		// differential rgb storage, trippy
/*		if( flAmount>1 )
		{
			Vector tmp = vecColor;
			VectorNormalize(tmp);
			flAmount *= vecColor.Length()/tmp.Length();
			tmp*=188;
			tmp[0]-=tmp[1];
			tmp[2]-=tmp[1];

			VectorFill(tmp, tmp[1]*flAmount);

			tmp[0]+=tmp[1]/flAmount;
			tmp[2]+=tmp[1]/flAmount;

			VectorAdd( m_vecLighting, tmp, m_vecLighting );
		}
		else
		*/
...
extern float	g_CloudAngularExtent;
